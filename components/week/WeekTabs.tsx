...
export default function WeekHeader({ selectedWeek, setSelectedWeek }: WeekTabsProps) {
        ...
        <div className="my-4">
            <ul className="flex h-full items-center gap-4">
                {weeks.map((weekId, idx) => (
                    <li
                        key={idx}
                        className={`inline-flex h-[40px] items-center rounded-full text-xl ${selectedWeek === weekId ? ' text-purple' : 'text-mint'} `}
                        onClick={() => handleClick(weekId)}
                        onDoubleClick={() => handleDoubleClick(weekId)}
                        style={{ zIndex: weeks.length - idx }}>
                        <input ref={inputRef} type="text" onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleWeekTitle(e)} value={weekState[weekId].title} className={`bg-transparent outline-none text-center w-[80px]`} disabled={editingWeekId !== weekId} onBlur={handleBlur} />
                    </li>
                ))}
            </ul>
        </div>
